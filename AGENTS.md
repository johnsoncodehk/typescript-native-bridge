...
- **Exceptions:** a crasher *or a silent wrong result* on the headline path may carry a stopgap patch — each must be listed in the README's tsgo-behavior-changes section with the upstream issue link, and removed once upstream lands the fix. **Every TNB change to tsgo behavior is listed in the README, and the list is gate-enforced against the patch contents.** A large alignment cost is never the default action — it needs maintainer sign-off.
